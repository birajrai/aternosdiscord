...
class iHalloweenBotForAternos {
    ...
    async #retrieveServerControlMessages() {
        ...
        for (let channel of channels.values()) {
            ...
            this.#serverControlMessages.push(
                ...messages
                    .filter(
                        msg =>
                            msg.author.id == this.#client.user.id &&
                            msg.embeds.length == 1 &&
                            msg.embeds[0].author.name == config.server + ' server control'
                    )
                    .values()
            );
        }
    ...
    async #buildControlMessage(guildIconURL) {
        ...
        serverControlEmbed.setAuthor({ name: config.server + ' Control Panel', iconURL: guildIconURL });
        serverControlEmbed.setFooter({
            text: "Please don't spam",
        });
        ...
        if (messageActionRow.components.length > 0) messageData['components'].push(messageActionRow);
    ...
    async #clientInteraction(interaction) {
        if (interaction.isApplicationCommand() && interaction.commandName == 'setup') {
...
exports.iHalloweenBotForAternos = iHalloweenBotForAternos;
