...
class Q9erig {
    ...
    async initialize() {
        ...
        await this.#serverPage.type('#user', config.username);
        await this.#serverPage.type('#password', config.password);
        ...
        await this.#serverPage.exposeFunction('onServerStatusChange', (newStatus, playerCount) =>
            this.#onServerStatusChange(newStatus, playerCount)
        );
    ...
    #validateAction(action) {
        return (
            (action == '#start' && this.serverStatus == 'Offline') ||
            (action == '#restart' && this.serverStatus == 'Online') ||
            (action == '#stop' && (this.serverStatus == 'Online' || this.serverStatus == 'Starting ...'))
        );
    }
    ...
    async executeServerAction(action) {
        ...
        if (action == '#start') {
            await this.#serverPage.waitForSelector(
                'div.alert > main > div.alert-body > div.alert-buttons.btn-group > a.btn.btn-red',
                { visible: true }
            );
            await this.#serverPage.click(
                'div.alert > main > div.alert-body > div.alert-buttons.btn-group > a.btn.btn-red'
            );
        }
    }
